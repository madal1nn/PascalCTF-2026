...
def receive_email(sender_address, pid) -> tuple[bool, str | None, TemporaryDirectory | None]:
    ...
    try:
        ...
        while time.time() - start < MAX_WAIT:
            # Select requests for this specific user
            session.select('inbox')
            # Check for specific subject match
            subject_query = f'{subject_prefix}{pid}'
            _, data = session.uid('search', 'UNSEEN', 'SUBJECT', f'"{subject_query}"')
