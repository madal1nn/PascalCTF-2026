...
class Character:
    ...
    def __convert_to_spectrogram(self, audio_channels=2, volume=0.4):
        upper_frequency_boundary = self.carrier_rate / 2 - self.carrier_rate / 50 + 2500
        lower_frequency_boundary = self.carrier_rate / 4 + 1500
        ...
        for i in range(len(raw_audio_data)):
            data = int(32767 * volume * raw_audio_data[i] / peak_frequency - 4500)
