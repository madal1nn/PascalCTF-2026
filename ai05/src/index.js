...
app.get('/getImage', setupSession, async function (req, res) {
  if (!req.session.cities) {
    req.session.paths = ['/images/Gubbio.png', '/images/OBlock.png', '/images/BollyWood.png']
    req.session.cities = ["Gubbio", "Chicago", "Bollywood"];
  }

  req.session.currentCity = req.session.cities[req.session.cities.length - 1]
  fileb64 = fs.readFileSync(path.join(__dirname, req.session.paths[req.session.cities.length - 1])).toString('base64');
  dataUrl = `data:image/png;base64,${fileb64}`;
...
app.post('/api/send', limiter, hasRequestStillPending, async function (req, res) {
  ...
  req.session.pending_response = false;
  if (ai_answer.includes("You've guessed it daddy!")) {
    req.session.currentGuesses++
    req.session.cities.pop();
    req.session.paths.pop();
  ...
  try {
    await chatsCollection.insertOne({
      ...
      currentCity: req.session.currentCity
    });
  } catch (err) {
    console.error('Failed to save chat to collection:', err);
  }

  res.json({ answer: ai_answer, guessed: req.session.guessed });
});
